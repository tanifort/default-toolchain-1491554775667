...
var appClientConfig = {
    ...
    "auth-key" : "a-2fbcv2-ysbdesrq2j",
    "auth-token" : "W+*cRFTjPdVp)-cOfB"
}
...
appClient.on("connect", function () {
    ...
    appClient.subscribeToDeviceEvents("iot-phone","kati","+","json");
...
var obj = JSON.parse(payload);
	...
	if ((xAxis>0.03 || xAxis<-0.03) && (yAxis>0.03 || yAxis<-0.03)){
		...
		console.log("Device is laying on table");
	}
	...
	var org_req = https.request(options, function(org_res) {
		...
		org_res.on('end', function() {
			try {
				...
				var url = "https://c...content-available-to-author-only...x.net/#/resources/serviceGuid=" + org.bluemix.serviceInstanceGuid + "&orgGuid=" + org.bluemix.organizationGuid + "&spaceGuid=" + org.bluemix.spaceGuid;
				res.json({ url: url });
